...
class GNStationClient(OpcUaClientWithSubscription):
    ...
    def _subscribe_once(self, nodes: Sequence[str]):
        """持久化订阅：整个设备实例复用同一个 subscription，watch 节点增量 add，绝不 delete。
        目的：每个 Device 只创建一次订阅，避免"每次 action 一个订阅/对话"的抖动。
        返回值统一为 None（不需要外部 delete）。"""
        with self._client_lock:
            # 首次调用：在同一 Session 上建立唯一的持久 subscription
            if getattr(self, "_persistent_sub", None) is None:
                handler = OpcUaClientWithSubscription.SubscriptionHandler(self)
                try:
                    self._persistent_sub = self.client.create_subscription(
                        self._subscription_interval, handler
                    )
                    self._persistent_sub_nodes = set()
                    logger.info(f"[{self.__class__.__name__}] 建立持久 OPC UA 订阅（复用于所有 _wait_done）")
                except Exception as exc:
                    logger.warning(f"建立持久订阅失败（降级为按需读）: {exc}")
                    self._persistent_sub = None
                    self._persistent_sub_nodes = set()
                    return None
            # 增量订阅：仅对未订阅的节点 add，避免同一节点多次订阅
            for name in nodes:
                chinese = self._name_mapping.get(name, name)
                if chinese in self._persistent_sub_nodes:
                    continue
                try:
                    ua_node = (
                        self._found_node_objects.get(chinese)
                        or self.client.get_node(self.use_node(chinese).node_id)
                    )
                    self._persistent_sub.subscribe_data_change(ua_node)
                    self._persistent_sub_nodes.add(chinese)
                except Exception as exc:
                    logger.warning(f"持久订阅节点 {chinese} 失败: {exc}")
        return None
    ...
    def _wait_done(
        ...
        """发指令后等到位：优先靠订阅推送（持久订阅），未启用/建订阅失败时降级按需读。"""
        watch = ([done_node] if done_node else []) + [c[0] for c in (reach_checks or [])]
        # 若未启用全局订阅，尝试在持久订阅上 add 当前 watch（幂等，已订阅则跳过）
        if watch and not self._use_subscription:
            self._subscribe_once(watch)
        # 触发前先清一次 watch 节点缓存，避免用上一轮的旧值（订阅未推送前）造成误判
        for name in watch:
        ...
        last_log = 0.0
        while time.time() - start < timeout:
            # 若持久订阅可用，优先用缓存；否则强制读服务器
            use_cache_first = getattr(self, "_persistent_sub", None) is not None or self._use_subscription
            values = {
                n: self.get_node_value(n, use_cache=use_cache_first, force_read=not use_cache_first)
                for n in watch
            }
            ok = True
            if done_node:
