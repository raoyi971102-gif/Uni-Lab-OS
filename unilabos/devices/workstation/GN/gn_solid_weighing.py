...
class SolidWeighingDevice(OpcUaClientWithSubscription):
    ...
    def _load_nodes_from_xlsx(self, xlsx_path: str) -> None:
        """从 opcua_gn CSV（Name/EnglishName/NodeType/DataType/NodeId）加载 Solid_ 前缀节点。"""
        try:
            ...
            logger.info(f"开始从 CSV 加载 Solid_ 节点: {xlsx_path}")
            all_nodes, name_mapping, reverse_mapping = self.load_csv(xlsx_path)

            # CSV 含全站节点，本设备只注册 Solid_ 前缀
            nodes = [n for n in all_nodes if str(n.name).startswith("Solid_")]
            name_mapping = {
                en: cn for en, cn in name_mapping.items()
                if str(en).startswith("Solid_") or str(cn).startswith("Solid_")
            }
            reverse_mapping = {
                cn: en for cn, en in reverse_mapping.items()
                if str(cn).startswith("Solid_") or str(en).startswith("Solid_")
            }

            if not nodes:
                logger.error("CSV 中未解析到任何 Solid_ 节点")
                return
            ...
            logger.info(f"✓ 成功从 CSV 加载 {found_count} 个 Solid_ 节点")
    ...
    def _reconnect_opcua(self) -> bool:
        """委托到基类幂等 _reconnect：先探活；活着直接返回 True 不动 Session，死了才重连并恢复订阅。"""
