...
class StackDevice(OpcUaClientWithSubscription):
    """旋转堆栈设备类（OPC 前缀 Stack_），结构同 SolidWeighingDevice / StandardOvenDevice。"""
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # 实例隔离节点表，避免站内其它设备全表 load 抢走 Stack_*
        self._node_registry = {}
    ...
    def _load_nodes_from_xlsx(self, xlsx_path: str) -> None:
        ...
        try:
            ...
            logger.info(f"开始从 CSV 加载 Stack_ 节点: {xlsx_path}")
            all_nodes, name_mapping, reverse_mapping = self.load_csv(xlsx_path)

            nodes = [n for n in all_nodes if str(n.name).startswith("Stack_")]
            name_mapping = {
                en: cn for en, cn in name_mapping.items()
                if str(en).startswith("Stack_") or str(cn).startswith("Stack_")
            }
            reverse_mapping = {
                cn: en for cn, en in reverse_mapping.items()
                if str(cn).startswith("Stack_") or str(en).startswith("Stack_")
            }

            if not nodes:
                logger.error("CSV 中未解析到任何 Stack_ 节点")
                return
            ...
            logger.info(f"✓ 成功从 CSV 加载 {found_count} 个 Stack_ 节点")
    ...
    def _reconnect_opcua(self) -> bool:
        ...
        if ok and self._last_reconnect_ts != prev_ts:
            try:
                ...
                logger.info("旋转堆栈 OPC UA 主动重连成功，已刷新节点句柄")
            except Exception as exc:
