...
class CentrifugeTubeLiquidHandlingDevice(OpcUaClientWithSubscription):
    ...
    def _load_nodes_from_xlsx(self, xlsx_path: str) -> None:
        """从 opcua_gn CSV（Name/EnglishName/NodeType/DataType/NodeId）加载 Tube_ 前缀节点。"""
        try:
            ...
            logger.info(f"开始从 CSV 加载 Tube_ 节点: {xlsx_path}")
            all_nodes, name_mapping, reverse_mapping = self.load_csv(xlsx_path)

            # CSV 含全站节点，本设备只注册 Tube_ 前缀
            nodes = [n for n in all_nodes if str(n.name).startswith("Tube_")]
            name_mapping = {
                en: cn for en, cn in name_mapping.items()
                if str(en).startswith("Tube_") or str(cn).startswith("Tube_")
            }
            reverse_mapping = {
                cn: en for cn, en in reverse_mapping.items()
                if str(cn).startswith("Tube_") or str(en).startswith("Tube_")
            }

            if not nodes:
                logger.error("CSV 中未解析到任何 Tube_ 节点")
                return
            ...
            logger.info(f"✓ 成功从 CSV 加载 {found_count} 个 Tube_ 节点")
