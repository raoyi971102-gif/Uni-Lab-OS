...
class XYZController(XYZStepperController):
            ...
            if os.path.exists(self.origin_file):
                with open(self.origin_file, 'r', encoding='utf-8') as f:
                    ...
                    for key, value in origin_data["machine_origin_steps"].items():
                        origin_data["machine_origin_steps"][key] = round(
                            self.mm_to_steps(MotorAxis[key.upper()], value), 2)

                    for key, value in origin_data["work_origin_steps"].items():
                        origin_data["work_origin_steps"][key] = round(
                            self.mm_to_steps(MotorAxis[key.upper()], value), 2)
    ...
    def work_to_machine_steps(self, x: float = None, y: float = None, z: float = None) -> Dict[str, int]:
        ...
        if x is not None:
            ...
            machine_steps['x'] = self.coordinate_origin.work_origin_steps['x'] + \
                work_steps + self.coordinate_origin.machine_origin_steps['x']

        if y is not None:
            work_steps = self.mm_to_steps(MotorAxis.Y, y)
            machine_steps['y'] = self.coordinate_origin.work_origin_steps['y'] + \
                work_steps + self.coordinate_origin.machine_origin_steps['y']

        if z is not None:
            work_steps = self.mm_to_steps(MotorAxis.Z, z)
            machine_steps['z'] = self.coordinate_origin.work_origin_steps['z'] + \
                work_steps + self.coordinate_origin.machine_origin_steps['z']
        ...
        try:
            ...
            for axis in MotorAxis:
                ...
                self.coordinate_origin.work_origin_steps[axis_name] = current_steps + \
                    machine_steps[axis_name] - self.coordinate_origin.machine_origin_steps[axis_name]
