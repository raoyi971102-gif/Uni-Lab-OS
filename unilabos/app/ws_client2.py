...
def _build_wss_ssl_context() -> ssl_module.SSLContext:
    """创建 WSS 上下文；Windows 证书库损坏时回退到 certifi。"""
    try:
        ...
        try:
            ...
            raise RuntimeError(
                f"系统证书库不可用，且未安装 certifi：{error}"
            ) from import_error
        logger.warning(
            f"[MessageProcessor] 系统证书库不可用 ({error})，改用 certifi CA 建立 WSS"
        )
        ...
        while self.is_running:
            try:
                ...
                if self.websocket_url.startswith("wss://"):
                    ssl_context = _build_wss_ssl_context()
